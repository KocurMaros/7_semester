...
Space=[ones(1,lstring)*(-u); ones(1,lstring)*pid_space];
Space(2,4) = 100;
Delta=Space(2,:)/100;   
    ...
    for i=1:lpop
        ...
        try
            ...
            % Fit(i) = sum(abs(out.e.Data)+abs(out.de.Data)+abs(out.y.Data));
                Fit(i) = sum(abs(out.e.Data).^2)+sum(abs(out.y.Data)); % +abs(out.u.Data)

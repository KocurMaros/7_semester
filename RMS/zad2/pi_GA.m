...
set_param('Regulator2','FastRestart','on')
...
for gen=1:numgen
    for i=1:lpop
        ...
        try
            out=sim("Regulator2");
            Fit(i) = sum(abs(out.e.Data) + b*abs(out.u.Data));
            % Fit(i) = sum(check_in_range(out.w_in.Data,out.w_out.Data,0.5));
            if(Fit(i) <e_min)
                BEST.Pop = Pop;
                BEST.param = param;
                BEST.i = i;
...
hold on
plot(out.u)
save("sims/GA/gear"+i_gear+"-pi.mat")
 % figure(1);
% plot(evolutions1);
% xlabel('generation');
% ylabel('fitness');
